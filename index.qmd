...
plot_streamfunction_anomaly(Date(2015, 11, 15))
```
...
if rescale_pcs
    ...
    pc_scores_scaled = Matrix(pc_scores) #Do this to prevent an error later in the code
end


println(typeof(pc_scores_scaled))
println(typeof(pc_scores))
```
...
- Explain in your own words what the `assign_to_nearest_centroid` function does and why it's important for k-means clustering.
    ...
    `assign_to_nearest_centroid` computes the euclidian distance from each point to each centroid, then finds the shortest distance for that point from the list of distances from that point to each centroid, and assigns it to the centroid that's closest to it. 

    This is important for k-means clustering because we are trying to group the points by the centroids they're closest to.  If we want to do this, we need to know which centroid that is. 


- Explain in your own words what the `update_centroids` function does and why it's important for k-means clustering.

    `update_centroids` finds the mean location of all points assigned to a given centroid, and then updates that centroid to this new mean location.  This is important for k-means clustering because we need our centroids to converge towards the center of each cluster, and this is what allows them to do that. 
...
- Why does k-means use squared Euclidean distance rather than absolute distance? What would happen if you used absolute distance instead?
    ...
    K-means uses squared Euclidean distance because this is how we accurately calculate the distance between 2 points in 2D Euclidean space.  If we were to use absolute distance, we might incorrecly compare the distances between points, and not obtain proper convergence. 
...
- Visualize streamfunction anomalies for at least 3 different days. Then, identify the weather types for each day. How well do individual days match their cluster composite patterns?
    ...
    December 15th, 2015 is similar to EOF 4, or Weather Type 2.  

    November 15th, 2015 is similar to EOF 3, or Weather Type 1

    November 30th, 2015 is arguably similar in structure to Weather Type 1, but doesn't seem to match any of the EOF's.

    Generally, days do seem to match their weather types, although some days match closer than others.  


- Do your EOF spatial patterns match those shown in the paper?

    Yes!  The EOF's in this lab appear to be a lower-resolution version of the four EOF's seen in figure 5 of the paper. 
...
- Do your weather type composites used with default settings (approximately matching the paper) match those in the paper? If not, why might they differ?
    ...
    Weather types 1 and 2 between the lab and the paper seem to have swithced places.  After that the weather types don't seem very similar, until we get to weather type 6. 
    
    With weather type 1 being 21.3% of days in the paper, and weather type 2 being 18.4%, it's possible that we converged towards slightly different clusters in the lab as we did in the paper.  Additionally, the paper utilized data from 1979 to 2016, while our lab used data from 1979 to 2025.  It's possible that with a few more years of data, we clustered slighly differently, especially if there were climatic non-stationarities that impacted the data in those last few years, or if ENSO swung one way more than the other during those years. 

- What is the effect of changing the number of retained PCs on the weather type patterns?

    If we use too few PC's, we can only look at the most dominant weather patterns, and we lose a lot of the variability in our data.  If we use too many, we may begin to overfit, or find principal components that don't necessarily reflect meaningful trends. 


- What is the effect of rescaling PCs before clustering? Try clustering without rescaling and compare results.

    Rescaling equalized the variance across principal components, so that we don't weight as heavily towards components that explain the most variance.  Although we are very interested in seeing which components explain the most variance, when we don't rescale, we end up with more than one similar looking weather types, and don't explain a diversity of trends as well as when we rescale.  There is some subtlety lost in the weather types that we can capture, since the one's were viewing are less distinct from each other.  The "big" clusteres dominate.


- What is the impact of changing the number of clusters (k) on the weather type patterns?


    Decreasing K lets us capture less unique trends.  We capture the more general weather patterns, but aren't able to identify all of the unique clusters as well, and some important clusters might not be detected. 

    Increasing K lets us capture more trends and have more information in our results, but if we do it too much, we might be trying to converge towards clusters that don't actually exist, or we might split meaningful and distinct clusters in half. 
